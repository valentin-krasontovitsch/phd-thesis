...
\newcommand{\myspan}[3]{\xymatrix{ #1 & #2 \ar[l] \ar[r] & #3 }}
...
\section{Iterated THH - Loday Functor}
  ...
  \begin{defn}\label{def_cateogry_of_spans}
    The category of spans $V$ has as objects the class of finite sets, while the morphism set $V(Y,X)$ for two finite sets $X,Y$ is given by the set of equivalence classes of diagrams of the form
    \[ \xymatrix{ Y & A \ar[l] \ar[r] & X }, \]
    also called spans. Two such diagrams $\myspan{Y}{A}{X}$, $\myspan{Y}{A^\prime}{X}$ are considered equivalent if there is a bijection $A \to A^\prime$ making the resulting triangles commutative.
  \end{defn}
