...
\subsection{The Loday functor for finite sets}
  ...
  \begin{lem}\label{lem_loday_functor_fixed_points}
    ...
    \begin{proof}
      ...
      % TODO make diagram on simplicial set level and append explanation for
      % last step
      This gives us the following commutative diagram:
      % OK TODO explain where morphism comes from
      % morphism of simplicial sets, defined degree wise
      % we should add that we use the iso J(S)^G cong J(S/G) in the term
      % to make sense of the hocolim over new index category
      % TODO should check that it respects the simplicial structure
      % OK TODO this will also show how the above is a bimodule...
      % it's not - taking fixed points is no good
      % TODO how is the lower horizontal morphism induced?
      % stupid answer - degree-wise
      % come up with a better answer!
      % better answer: after switching to "same" situation you have the
      % same map - now we just have to check that the resulting square commutes
      % OK TODO how can we apply the same stuff on the RHS?
      % OK TODO does the equality of pulling ^G in hold there as well?
      % yes it does. 
      % OK TODO is \cJ(S)^G \cong \cJ(S/G)?
      % Yes, it is - at least on the level of objects.
      % Not so sure about morphisms though...
      % depends on def of fixed points, but with strict def it's OK
      % OK TODO why are we now in the situation from before?
      % cause we can put a ^G in the index, pull both G's out, and voila
      \begin{displaymath}
        \xymatrix@C+2em{
