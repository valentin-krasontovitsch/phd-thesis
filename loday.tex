...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{The left lax transformation $G^A$}
    ...
    \begin{defn}\label{def_left_lax_transformation_S}
      ...
      We choose a strong symmetric monoidal functor $S: \Sigma \to \sS_*$ with $S(\{1\}) = S^1$ (one possible such choice is to set $S(X) = \bigwedge_{x \in X} S^1$ for any finite set $X$).
    ...
    \begin{rem}\label{rem_notation_for_functor_S}
      Considering the above functor $S: \Sigma \to \sS_*$, we will write $S^X \defas S(X)$, intending to indicate a sphere of dimension cardinality of $X$, as we have a homeomorphism
      \begin{displaymath}
        S(X) = S( \coprod_{x \in X} \{x\}) = \bigwedge_{x \in X} S(\{x\}) \cong %
          \bigwedge_{x \in X} S(\{1\}) = \bigwedge_{x \in X} S^1.
      \end{displaymath}
    \end{rem}
    ...
    \begin{defn}\label{def_left_lax_transform_G^A}
      ...
      \begin{gather*}
        ...
        (i \colon T \to \cI) \mapsto %
          \Hom(\bigwedge_{t \in T} S^{i(t)}, \bigwedge_{t \in T} A_{i(t)} )
      \end{gather*}
      which sends a morphism $(\alpha_t \colon i(t) \to j(t))_{t \in T}$ in $\cI^T$ to the map
      \begin{displaymath}
        \xymatrix{
          \Hom(\bigwedge\limits_{t \in T} S^{i(t)}, \bigwedge\limits_{t \in T} A_{i(t)})
          ...
          \Hom(\bigwedge\limits_{t \in T} S^{i(t)} \wedge %
            ...
            \bigwedge\limits_{t \in T} A_{i(t)} \wedge %
            \bigwedge\limits_{t \in T} S^{j(t) \setminus \im \alpha_t})
            \ar[d]^-{\cong} \\
          \Hom(\bigwedge\limits_{t \in T} S^{i(t)} \wedge S^{j(t) \setminus \im \alpha_t}, %
            \bigwedge\limits_{t \in T} A_{i(t)} \wedge S^{j(t) \setminus \im \alpha_t}) 
            \ar[d]^-{(\cong, \bigwedge\limits_{t \in T} \sigma^A_{i(t),j(t)\setminus\im\alpha_t})} \\
          \Hom(\bigwedge\limits_{t \in T} S^{j(t)}, \bigwedge\limits_{t \in T} A_{i(t) \coprod (j(t)\setminus \im \alpha_t)})
            \ar[d]^{(\bigwedge\limits_{t \in T} \chi_t)_*} \\
          \Hom(\bigwedge\limits_{t \in T} S^{j(t)}, \bigwedge\limits_{t \in T} A_{j(t)})
        }
      \end{displaymath}
      where the second map is induced by permutation isomorphisms, the third map is induced by the isomorphism $S(j(t) \setminus \im \alpha_t \coprod i(t)) \cong S(j(t))$ in the first variable and the respective structure maps in the second variable, and the last map is induced by the action of the shuffle permutation $\chi_t \colon \im \alpha_t \coprod (j(t)\setminus \im \alpha_t) \to j(t)$ rearranging the elements into order (recall that the elements of $\cI$ are sets of the form $\{1 , \ldots, n\}$, i.e. we may consider them ordered).\\
      This defines a morphism of symmetric spectra as we use morphisms of pointed simplicial sets and functoriality of the $\Hom$ functor.
      It is functorial: Let $\beta \colon j \to k$ be another morphism in $\cI^T$.
    \end{defn}
