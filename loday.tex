...
\section{Iterated THH - Loday Functor}
  ...
  \begin{defn}\label{def_functor_iW_to_Cat_aka_abstract_loday}
    ...
    Note that we could not define this functor for instance with domain $W$ or $eW$, as we only assumed $\cC$ to have all isomorphisms of $\cB$: The coproducts we form are formed in $\cC$, and by assuring that the maps we obtain by using the properties of the coproduct (in $\cB$!) are isomorphisms we may assume that they are already in $\cC$.\\
    %
