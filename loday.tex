...
\section{Iterated THH - Loday Functor}
  ...
  \begin{defn}\label{def_J_and_functors_from_finite_sets_to_pointed_categories}
    Consider the full subcategory $\cF \subset \Fin$ of finite sets of the from $\ind{n} = \{ 1, \ldots , n\}$. We choose a finite coproduct in $\cF$ by taking the sum of two sets: More precisely, given integers $n,m > 0$, we let $\ind{n} \sqcup \ind{m} \defas \ind{n+m}$ with inclusions given by mapping the first summand $\ind{n}$ identically onto the first $n$ elements, i.e. 
    \begin{displaymath}
    ...
    \end{displaymath}
    \comm{This does not give a well-defined coproduct for arbitrary finite sets, only for well-ordered ones?}\\
    ...
    % TODO substitute the categories with the actual objects!
      ...
      \left[ \coprod_{(x,f(x),gf(x))} c_x \right]_{z \in Z} \crightarrow{2}{} %
