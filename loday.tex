...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{The left lax transformation $G^A$}
    ...
    \begin{defn}\label{def_left_lax_transform_G^A}
      ...
      \end{displaymath}
      where the second map is induced by permutation isomorphisms, and the third map is induced by the isomorphism $S(j(t) \setminus \im \alpha_t \coprod i(t)) \cong S(j(t))$ in the first variable and the respective structure maps in the second variable.\\
      This defines a morphism of symmetric spectra, as we just use morphisms of pointed simplicial sets and functoriality of the $\Hom$ functor.
      It is functorial: Let $\beta \colon j \to k$ be another morphism in $\cI^T$.
      \comm{last map should require a permutation action: how are $i(t)$ and $j(t) \setminus \im \alpha_t$ arranged?}\\
    \end{defn}
