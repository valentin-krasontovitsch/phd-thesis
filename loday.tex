...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{The left lax transformation $G^A$}
    ...
    \begin{defn}\label{def_left_lax_transform_G^A}
      ...
      We combine the two above definitions to obtain a left lax transformation $G^A \colon \cJ \to \Sp^\Sigma$ for a \hring~$A$, where $\cJ = \cJ(-_*) \colon \Fin \to \CAT_*$ is the covariant weak functor defined in Def.~\ref{def_J_and_functors_from_finite_sets_to_pointed_categories} (postcomposed with the inclusion $\Cat_* \to \CAT_*$), and $\SpS$ denotes the constant functor with value in symmetric spectra of simplicial sets:\\
  ...
  \subsection{The Loday functor for finite sets}
    ...
    \begin{defn}\label{def_rectification_of_G^A}
      Consider the functor $\tilde{\cJ} \colon V \to \Cat_*$ and the left lax transformation $\tilde{e} \colon \tilde{\cJ} \to \cJ$ which we obtain by applying Street's first construction (the pointed version, cf. Defs.~\ref{def_streets_first_construction_functor},~\ref{def_streets_first_construction_transformation}) in the context $\cD \defas V$ and $F \defas \cJ$, and let $r \defas e^{\op} \wedge e \colon \tilde{\cJ}^e \to \cJ^e$.
    \end{defn}
