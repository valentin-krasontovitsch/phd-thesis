...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{Left lax transformations, bimodules and homotopy colimits}
    ...
    \begin{defn}\label{def_category_of_bimodules}
      Let $\cJ$ be a small category, and let $K: \cJ \to Cat_*$ be a functor. Recall the following notation: Given $F: \cJ \to \Cat_*$, we denote by $J^e: \cJ \to \Cat_*$ the functor sending $j \mapsto F(j)^e = F(j)^{\op} \wedge F(j)$. We proceed to define the category $\operatorname{Bimod}^J / K$ of bimodules \comm{[TODO: analogy bimodules over (commutative) ring?]}. The objects of this category are pairs $(F,G)$, where $F:\cJ \to \Cat_*$ is a functor and $G: F^e \Rightarrow K$ is a left lax transformation (compare Rem. \ref{rem_left_lax_transformation}).
