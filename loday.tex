...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{The left lax transformation $G^A$}
    ...
    \begin{rem}\label{rem_notation_for_functor_S}
      ...
      \begin{displaymath}
        S(X) = S( \coprod_{x \in X} \{x\}) = \bigwedge_{x \in X} S(\{x\})
      \end{displaymath}
      \comm{The first equality is not an equality, only an iso! $X \neq \coprod_{x \in X} \{x\}$}\\
      \comm{Have a chosen coproduct on $\cF$, skeletal category - does $S$ need to be defined on $\Sigma$?}\\
      and the bijection $\{1\} \cong \{x\}$ for $x \in X$ induces a homeomorphism $S^1 = S(\{1\}) \cong S(\{x\})$. Using this homeomorphism and the above equality we may thus interpret $S(X)$ as a sphere of dimensionality $\abs{X}$ the cardinality of $X$. This will often be implicit.
    \end{rem}
