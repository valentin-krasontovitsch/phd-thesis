...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{The left lax transformation $G^A$}
    ...
    \begin{defn}\label{def_left_lax_transform_G^A}
      Given two pointed simplicial sets $X$ and $Y$, we define a \emph{symmetric spetrum} of functions $\Hom(X,Y)$ given in degree $n$ by the internal hom object of pointed simplicial sets $\Hom_{\sS_*}(X,Y \wedge S^n)$, $\Sigma_n$ acting on the $n$-sphere, and structure maps of assembly type
      \begin{gather*}
