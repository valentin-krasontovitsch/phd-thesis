...
\section{Iterated THH - Loday Functor}
  ...
  \begin{defn}\label{def_cateogry_of_spans}
    The category of spans $V$ has as objects the class of finite sets, while the morphism set $V(X,Y)$ for two finite sets $X,Y$ is given by the set of equivalence classes of diagrams called spans of the form $X \ot A \to Y$. Two such diagrams $X \ot A \to Y$, $X \ot A^\prime \to Y$ are said to be equivalent if there is a bijection $A \to A^\prime$ making the resulting triangles commute. To compose two maps represented by $[X \ot A \to Y]$ and $[Y \ot B \to Z]$, we take the pullback $A \ot C \to B$ of $A \to Y \ot B$ and compose with the maps $X \to A$, $B \to Z$ to obtain $[X \ot C \to W]$.
  ...
  \begin{defn}\label{def_bicategory_functor}
    ...
    \begin{itemize}
      ...
      \item for each composable pair of 1-morphisms $(f,g)$ in $\cC$ a 2-morphism $\phi_{g,f}:F(g)\circ~F(f)\to~F(g\circ~f)$,
