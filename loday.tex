...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{Rectification}
    ...
    \begin{defn}\label{def_streets_first_construction}
      ...
      \end{displaymath}
      which we interpret as an object of the category $x/e_d$.
    \end{defn}

    \begin{defn}\label{def_comma_category}
      Given two functors $F:\cC \to \cD$ and $G: \cE \to \cD$, the comma category $(F \downarrow G)$ is defined as  ...
      % TODO write def
      % TODO move to right place
    \end{defn}

    \begin{defn}\label{def_cofinal}
      A functor $F: \cC \to \cD$ is called cofinal if ...
      % TODO write def
      % TODO move to right place
    \end{defn}

    \begin{lem}\label{lem_functor_cofinal_iff_comma_categories_have_initial_object}
      Let $F:\cC \to \cD$ be a functor. Then $F$ is cofinal if and only if for every $d \in \ob \cD$ the comma category $(d \downarrow F)$ has an initial object.
      % TODO write lemma
      % TODO prove or find ref
      % TODO move to right place
    \end{lem}
