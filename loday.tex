...
\section{Iterated THH - Loday Functor}
  ...
  \begin{defn}\label{def_bicategory_functor}
    ...
    \end{gather*}
    A colax functor may be defined dually. A functor (in particular a lax functor) between bicategories is called a strict or weak functor if the structure maps $\phi_{g,f}$ and $\phi_a$ are isomorphisms or identity morphisms, respectively, for all composable $g,f$ and all $a \in \cC_0$.
    % TODO remove remark about colax - not in article, not necessary
  \end{defn}
  ...
  \begin{lem}\label{lem_discrete_bicategory}
    Given a category $\cC$, we may define a bicategory $\cC$ by taking the 0-cells to be the objects of $C$, and declaring the morphism class to be a discrete category. Composition and identity 1-arrows are composition and identity morphisms in $\cC$, structure morphisms are equalities, hence the axioms of a bicategory are trivially satisfied. 
  \end{lem}
