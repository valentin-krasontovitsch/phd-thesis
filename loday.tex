...
\section{Iterated THH - Loday Functor}
  ...
  \begin{defn}\label{def_pointed_stuff}
    ...
    Given two pointed categories $\cC$ and $\cD$, we may define another pointed category $\cC \wedge \cD$ by taking the smash products of object and morphism sets, respectively. It is easy to convince oneself that this again constitutes a pointed category.\\
    We would like to note a particular example at this point that we will come to use later. Given a pointed category $\cC$, we let $\cC^e \defas \cC^{\op} \wedge \cC$. This might remind one of the notation for the enveloping algebra $A^{e} = A^{\op} \otimes A$ of a unital and associative algebra, and indeed there is a connection which we point out in Rem.~\ref{rem_connection_to_algebras}.
  \end{defn}
