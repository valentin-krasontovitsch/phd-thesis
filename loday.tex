...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{Left lax transformations, bimodules and homotopy colimits}
    ...
    \begin{rem}\label{rem_left_lax_transformation}
      ...
      % TODO (luxury) move 2-arrow indicators a bit over, have done it in relations i think
      \begin{displaymath}
        \xymatrix{
          F(b)
            \ar[rr]^-{\sigma_b}
            ...
            \ar@{}[rdd]|*\objectbox{\overset{\phi^F_{g,f}}{\Leftarrow}}
          &
          %
          &
          F^\prime(b)
      ...
      \begin{displaymath}
        \xymatrix{
          ...
          F^\prime(b)
            \ar[dr]^{F^\prime(f)}
            \ar[dd]_{F^\prime(gf)}
            \ar@{}[rdd]|*\objectbox{\overset{\phi^{F^\prime}_{g,f}}{\Leftarrow}}
          &
          %
          \\
          %
    ...
    \begin{rem}\label{rem_left_lax_transform_functors_J_to_Cat}
      We further inspect the definition of left lax transformation and specify to the situation of two functors $E,F: \cJ \to \Cat_*$ for some small category $\cJ$. Then a left lax transformation  what does it mean to be a left lax transformation $E \Rightarrow F$?
    \end{rem}
