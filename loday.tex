...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{The left lax transformation $G^A$}
    ...
    \begin{defn}\label{def_left_lax_transform_G^A}
      ...
      \begin{displaymath}
        \Hom(X,Y) \in \ob \SpS
      \end{displaymath}
      given in degree $n$ by the internal hom-object of pointed simplicial sets $\Hom_{\sS_*}(X,Y \wedge S^n)$ \comm{[TODO: fibrant replacement codomain!]}, $\Sigma_n$ acting on the $n$-sphere, and structure maps of assembly type
      \begin{gather*}
  ...
  \subsection{The Loday functor for finite sets}
    ...
    \begin{defn}\label{def_rectification_of_G^A}
      ...
      \begin{displaymath}
      [G^A \circ p_2 \circ r] \colon (\iota \circ \tilde{\cJ}^e \circ (-)_*)\to \Sp^{\Sigma}
      \end{displaymath}
      of functors
      \begin{displaymath}
        \Fin \to \CAT_*.
      \end{displaymath}
      We introduce the notation $\cG^A \defas G^A \circ p_2$ for the sake of brevity. Note that this precisely constitutes an object  we have obtained ... What? \comment{TODO:} We should now be able to describe a functor from $\Fin$ to some category of bimodules. Do this here!?
    \end{defn}
    We are finally ready to formulate the following
