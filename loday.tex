...
\section{Iterated THH - Loday Functor}
  ...
  % TODO add remark about composition of 2-functors being a 2-functor
  ...
  \begin{defn}\label{def_J_and_functors_from_finite_sets_to_pointed_categories}
    Consider the full subcategory $\cF \subset \Fin$ of finite sets of the from $\ind{n} = \{ 1, \ldots , n\}$. We choose a coproduct in $\cF$ by taking the sum of two sets: More precisely, given integers $n,m > 0$, we let $\ind{n} \sqcup \ind{m} \defas \ind{n+m}$ with inclusions given by mapping the first summand $\ind{n}$ identically onto the first $n$ elements, i.e. $\ind{n} \to \ind{n+m}, i \mapsto i$, and mapping the second summand $\ind{m}$ identically onto the last $m$ elements, i.e. $\ind{m} \to \ind{n+m}, j \mapsto n+j$.\\
    We let $\cI$ be the subcategory of $\cF$ given by all inclusions (in particular, all objects of $\cF$ are in $\cI$) and apply the constructions described in Def. \ref{def_functor_iW_to_Cat_aka_abstract_loday} (abstract Loday functor) with $\cC \defas \cI$ as well as Def. \ref{def_extending_functor_iW_to_Cat_to_pointed_version} (adding a basepoint) to obtain a functor 
    \begin{displaymath}
    ...
    \begin{gather*}
      ...
      (c: X \to \cI) \mapsto \left\{ y \mapsto \coprod_{x \in f^{-1}(y)} c(x) \right\}
    \end{gather*}
