...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{The left lax transformation $G^A$}
    ...
    \begin{defn}\label{def_left_lax_transformation_S}
      ...
      \end{displaymath}
    where $\Sigma_+ \colon \Fin \to \CAT_*$ is the weak functor obtained by applying Def. \ref{def_functor_iW_to_Cat_aka_abstract_loday} to the inclusion $\Sigma \to \Fin$ and Def. \ref{def_added_basepoint_extending_functor} to the resulting functor, and finally precomposing with the functors $\Fin \myrightarrow{(-)_*} V \myrightarrow{L} iW \myrightarrow{\iota} eW$ (cf. Def. \ref{def_functors_finite_sets_to_iW}) and , while $\sS_*$ is used to denote the constant functor with image pointed simplicial sets.
  ...
  \subsection{Rectification}
    ...
    \begin{defn}\label{def_streets_construction_right_transformations}
      ...
      Given a morphism $f \colon d \to d^\prime$, recall that $\tilde{F}(f)$ was given by post-composition in the first entry on objects and the identity on morphisms, while $\tilde{\rho}_d$ only acts on the second entry on objects, and is (in particular) independent of $d$ for morphisms.
    \end{defn}
    %
