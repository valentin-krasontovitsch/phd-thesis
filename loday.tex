...
\section{Iterated THH - Loday Functor}
  ...
  % TODO add name 2-functor, and use it!
  \begin{defn}\label{def_bicategory_functor}
    Given two bicategieres $\cC$, $\cD$ a lax functor $F = (F, \phi): \mathcal{C} \to \mathcal{D}$ consists of
    \begin{itemize}
  ...
  \begin{defn}\label{def_subcategories_of_epimorphisms_and_isomorphisms}
    We define two sub-biactegories $iW \subset eW \subset W$ differ from $W$ only in the 2-morphisms: while $eW$ only permits those 2-morphisms in $W$ that are epimorphisms, $iW$ only permits isomorphisms.
  \end{defn}
  ...
  \begin{defn}\label{def_functor_iW_to_Cat}
    We let $\cB$ be a category with chosen finite coproducts, and we let $\cC \subset \cB$ be a subcategory that contains all isomorphisms in $\cB$, in particular it has all objects of $\cB$. We proceed to define a weak functor of bicategories
    \begin{displaymath}
      \cC: iW \to \Cat
    \end{displaymath}
    by sending a finite set $X$ to $\cC(X) \defas \cC^X$, the $X$-fold product of $\cC$ with itself. Given a span $f = (X \myleftarrow{f_1} A \myrightarrow{f_2} Y)$, we define a functor
    \begin{displaymath}
      \cC(f): \cC(X) = \cC^X \to \cC^Y = \cC(Y)
    \end{displaymath}
    by assigning to an object $(c: X \to \ob \cC) \in \ob \cC^X$ (a tuple, written as a map) the object 
    \begin{displaymath}
    ...
    %
    As $\Cat$ is a strict category, it is straight-forward to believe that the axioms for a functor of bicategories are satisfied by the just defined structure.
  \end{defn}
  ...
  \begin{defn}\label{def_added_basepoint_extending_functor}
    Consider the bicategory of small pointed categories $\Cat_*$ with functors and natural transformations, and the (strict) forgetful functordown to categories $\Cat_* \to \Cat$. This functor has a left adjoint, denoted
    \begin{displaymath}
      (-)_+ : \Cat \to \Cat_*,
    \end{displaymath}
    sending a category $\cC$ to the category $\cC_+$ which compared to $\cC$ has an extra object $0$, the adjoint basepoint, and extra morphisms $c \to 0$ and $0 \to c$ for every object $c \in \cC_0$. Functors and natural transformations of categories are uniquely extended to their respective pointed versions.
    % forgetful functor bifunctor: yes! adjoint functor bifunctor? check! is it a strict functor? check the adjointness! Integrate this into above definition? <- TODO
  \end{defn}

  \begin{def}\label{def_extending_functor_iW-to-Cat_to_eW-to-Cat_*}
    Given a weak functor of bicategories $F: iW \to \Cat$, we would like to extend this functor along the inclusion $\iota: iW \to eW$ as displayed in the following diagram:
    \begin{displaymath}
      \xymatrix{
        iW
          \ar[d]_{F}
    ...
    \end{displaymath}
    where the bottom arrow refers to the functor found in Def. \ref{def_pointed_stuff}. The value of $F_+$ on objects, 1-arrows and invertible 2-arrows is determined if we ask for the above diagram to commute. Hence we need merely indicate the images of non-invertible 2-arrows in $eW$ under $F_+$.
  \end{def}
