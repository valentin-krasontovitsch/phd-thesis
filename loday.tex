...
\section{Iterated THH - Loday Functor}
  ...
  \begin{defn}\label{defn_group_acting_on_object_in_V_morphism_phi}
    Given a finite set $X$ with a left action of a group $G$, we define the action on $X$ through automorphisms in $V$ by mapping $g \mapsto [X \myleftarrow{\id_X} X \myrightarrow{g} X]$.
  \end{defn}
