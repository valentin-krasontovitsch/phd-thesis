...
\section{Iterated THH - Loday Functor}
  ...
  \subsection{Left lax transformations, bimodules and homotopy colimits}
    ...
    \begin{defn}\label{def_hocolim}
      ...
      For functoriality in $\Delta^op$, we specify a simplicial structure as follows. Degenracy maps are induced by first adding a smash factor $S^0$, and then using the pointed map $S^0 \to \hom_{F(j)}(x_i,x_i)$ mapping the non-basepoint to the identity in the morphism set.
